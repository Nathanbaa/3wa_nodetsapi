...
export const signup = async (
  ...
  try {
    ...
    if (newCustomer) {
      ...
      // Envoyer les données au serveur client (front-end)
...
export const login = async (
  ...
  try {
    ...
    if (customer) {
      const isPasswordValid = await isValidatePassword(
        password,
        customer.password,
        customer.salt
      );
      ...
      if (isPasswordValid) {
        ...
        return res
          .status(200)
          .json({ success: true, data: signature, error: null });
      }
    }
    return res.status(403).json({
      success: false,
      data: null,
      error: { message: "Invalid credentials", code: 409 },
    });
  } catch (error) {
    next(error);
  }
    ...
    if (customer) {
      ...
      if (profile) {
        ...
        return res.status(400).json({
          success: false,
          data: null,
          error: { message: "Erreur avec l'OTP saisi", code: 400 },
        });
      }
    }
  } catch (error) {
...
export const requestOTP = async (
  ...
  try {
    ...
    return res.status(400).json({
      success: false,
      data: null,
      error: { message: "Error with request OTP", code: 400 },
    });
  } catch (error) {
    next(error);
  }
...
export const getCustomerProfile = async (
  ...
  try {
    ...
    return res.status(400).json({
      success: false,
      data: null,
      error: { message: "Error with fetch profile", code: 400 },
    });
  } catch (error) {
    next(error);
  }
...
export const updateCustomerProfile = async (
  ...
  try {
    ...
    if (customer) {
      ...
      return res
        .status(200)
        .json({ success: true, data: updatedCustomer, error: null });
    }
    return res.status(400).json({
      success: false,
      data: null,
      error: { message: "Error with editing profile", code: 400 },
    });
  } catch (error) {
    next(error);
  }
